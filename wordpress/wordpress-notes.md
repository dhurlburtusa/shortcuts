...
- The navigation menus are stored in the `wp_term_taxonomy` table with a `taxonomy` of `nav_menu` and in the `wp_term` table joined to the `wp_term_taxonomy` table by `term_id`.
